...
public class SensitiveService implements InitializingBean {
    ...
    private class TireNode {
        ...
        public void addSubNode(Character key, TireNode node) { //添加子节点
            subNodes.put(key, node);
        }

        public TireNode getSubNode(Character key) { //得到子节点
            return subNodes.get(key);
        }

        public boolean isKeywordEnd() {  //判断是否为尾节点
            return end;
        }

        public void setkeywordEnd(boolean end) {  //设置尾节点
            this.end = end;
        }
    }

    private TireNode rootNode = new TireNode(); //建立前缀树
